...
final class LoginViewController: UIViewController, UITextFieldDelegate {
    ...
    private lazy var emailTextField: UITextField = {
        let textField = createTextField(text: "学号")
    ...
    private lazy var passwordTextField: UITextField = {
        let textField = createTextField(text: "密码")
    ...
    @IBAction func loginTMS(_ sender: Any) {
        ...
        AF.request("http://localhost:6226/ios",method: .get, parameters: parameters).responseJSON { response in
            ...
            if let value = response.result.value{
                let json = JSON(value)
                let filePath = NSHomeDirectory() + "/Preferences/Schedule.plist"
                let saveData = NSArray.init(objects:json)
